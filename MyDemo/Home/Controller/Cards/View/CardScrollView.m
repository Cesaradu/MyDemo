...
- (void)loadBottomView {
    ...
    for(int i=0; i<(_cardDataArray.count<4?_cardDataArray.count:4); i++) {
        ...
        //实例化CardView
        CardView *card = [[CardView alloc]initWithFrame:CGRectMake(point.x, point.y, width,height-30)];
        card.backgroundColor = [UIColor whiteColor];
        card.layer.masksToBounds = YES;
        card.layer.cornerRadius = 5.0;
        [card loadCardViewWithData:_cardDataArray[i]];
...
-(void)loadSlideCardViewWithCount:(NSInteger)count {
    ...
    if (_cardDataArray.count>10) {
        ...
        card.frame = CGRectMake(0, _index* viewSize.height, viewSize.width, viewSize.height-30);
    ...
    //实例化CardView
    CardView *card = [[CardView alloc]initWithFrame:CGRectMake(point.x, point.y, viewSize.width, viewSize.height-30)];
    card.backgroundColor = [UIColor whiteColor];
    card.layer.masksToBounds = YES;
    card.layer.cornerRadius = 5.0;//设置圆角
    [card loadCardViewWithData:_cardDataArray[0]];
...
{
    if([self.delegate respondsToSelector:@selector(cardScrollViewDidSlectIndex:)])
    {
        [self.delegate cardScrollViewDidSlectIndex:_index];
    }
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView //滚动时处理
{
    ...
    if(scrollCardView.frame.origin.y<offset_y)
    {
        if(offset_y>_cardDataArray.count*height-height){
            ...
            [scrollCardView loadCardViewWithData:_cardDataArray[_cardDataArray.count-1-_index]];
        }
    }
    ...
    for (NSInteger i=_select; i<=_index; i++) {
        ...
        [moveCardView loadCardViewWithData:_cardDataArray[_cardDataArray.count-1-i]];
        
        float range_y = (currOrigin_y - offset_y)/(_xMarginValue) ;
        
        moveCardView.frame = CGRectMake(0, range_y, width, height-30);
    ...
    //代理滚动时回调函数
    if([self.delegate respondsToSelector:@selector(cardScrollViewDidScrollAllPage:AndIndex:)])
        [self.delegate cardScrollViewDidScrollAllPage:_cardDataArray.count-1 AndIndex:_index];
}
...
{
    if([self.delegate respondsToSelector:@selector(cardScrollViewDidEndScrollIndex:)])
    {
        [self.delegate cardScrollViewDidEndScrollIndex:_index];
    }
}
