...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        //标题
        self.title = [UILabel labelWithTitle:@"标题标题标题标题标题标题标题标题标题标题标题标题标题标题标题标题" AndColor:@"8a8a8a" AndFont:15 AndAlignment:NSTextAlignmentLeft];
        self.title.numberOfLines = 2;
        [self.bgView addSubview:self.title];
        [self.title mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.bgView.mas_top).offset([self Suit:260]);
            make.left.equalTo(self.bgView.mas_left).offset([self Suit:15]);
            make.width.mas_equalTo(self.frame.size.width);
        }];
        
        //价格
        self.price = [UILabel labelWithTitle:@"价格: ¥10000.00" AndColor:@"515151" AndFont:14 AndAlignment:NSTextAlignmentLeft];
