...
#define AnimationDISTANCE  -64
#define NavigationHeight 64

@interface PMElasticView () <CAAnimationDelegate>
...
- (void)configSubViews {
    ...
    self.elasticShaperLayer.fillColor = [UIColor lightGrayColor].CGColor;
...
- (CGPathRef)calculateAnimaPathWithOriginY:(CGFloat)originY {
    ...
    CGPoint bottomLeftPoint = CGPointMake(0, self.offSet_Y <= AnimationDISTANCE ? ABS(AnimationDISTANCE) : originY);
    CGPoint controlPoint = CGPointMake(self.bindingScrollView.bounds.size.width * .5, originY);
    CGPoint bottomRightPoint = CGPointMake(self.bindingScrollView.bounds.size.width, self.offSet_Y <= AnimationDISTANCE ? ABS(AnimationDISTANCE) : originY);
